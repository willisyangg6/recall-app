...
- **Legacy rows (`apply_state IS NULL` = legacy_unverified).** Rows predating the marker keep exactly the pre-O3 behavior on unchanged content — a snapshot's existence is never taken as proof it was applied, but the historical corpus also never replays through notification paths merely because the new code exists. A legacy record graduates to a verified marker when its content actually changes (the normal apply path), or through the separately authorized O3-B2 reconciliation — implemented as a governed, plan-bound census (dry-run default; seeding requires a reviewed immutable plan + digest; every historical mismatch is refused and enumerated, never auto-repaired; marker seeding writes only the four marker fields and zero notification events). Operational contract and runbook: [docs/recall-operations.md](recall-operations.md).

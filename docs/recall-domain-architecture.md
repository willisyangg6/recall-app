...
```
 ...
 openfda_enforcement ─┘  (poll daily)
        ...
        ▼
   applied-version gate  (unchanged ⇔ latest archived version == APPLIED version)
        ▼
   archive-and-pending  (archive_snapshot: hash-gated append + record → pending, one transaction)
        ▼
   parse + normalize → NormalizedSourceRecord (one shared shape, quirk-free)
        ▼
   identity resolution (FDA linking Part 6; FSIS -EXP prefix linking; PHA retraction linking)
        ▼
   ...
   material-change detection (Part 9: diff of consumer-relevant projection)
        ▼
   ONE atomic case transition  (apply_case_transition / found_recall_case:
   projection + timeline + affected products + NotificationEvents + applied markers)
```
...
- **Idempotency:** snapshots are hash-gated (refetching identical content is a no-op _once applied_ — see crash safety below); SourceRecord upserts key on native ID; canonical projection is a pure function of current linked records (re-running changes nothing); notifications dedup on `dedupKey` (Part 10). The whole pipeline can be re-run from snapshots at any time — which is also the parser-improvement path (Part 12).
- **Crash safety — the applied-version contract (O3-B1).** The pre-O3 gate compared the incoming content hash against the newest _archived_ snapshot while the snapshot was written _before_ the normalized record, projection, products, and notification events — so a crash between archive and apply left every later run treating the version as processed: permanently stale consumer data behind a green run (the defect proven in `.reports/o3-a-atomicity-audit.json`). The contract now separates two durable facts on `source_records` — `apply_state` / `applied_content_hash` / `applied_snapshot_seq` / `applied_at` — and the gate skips only when the incoming hash equals the latest archived snapshot AND that snapshot is marked applied. Transaction boundaries: `archive_snapshot` archives and marks the record `pending` in one transaction (rejecting an older `fetched_at` before any write, so a stale worker never regresses anything); the whole consumer-visible transition — case projection, timeline, affected-product replacement, notification events, and the applied markers of every source version it carries — commits through `apply_case_transition` (or, for a brand-new record, `found_recall_case`, which founds case + record + snapshot + products + initial event + marker atomically, so an orphan case can no longer exist) in ONE transaction with a compare-and-swap on the case's `last_changed_at`. A CAS loser re-reads all contributing records, recomputes (deterministic, Part 8.4), and retries up to 3 times; exhaustion leaves the version visibly `pending` for the next tick. Everything between archive and apply is therefore pending-and-retryable; a consumer can never observe a torn state, and a required notification can never commit without its case version or be lost after it. The normalized source-record write remains a separate, sequence-monotonic conditional update _before_ the case transition; the accepted (bounded) tradeoff is that a sibling record's reprojection can momentarily read a normalized payload whose own case transition has not yet committed — the same data that transition applies moments later, never a torn consumer state.
- **Legacy rows (`apply_state IS NULL` = legacy_unverified).** Rows predating the marker keep exactly the pre-O3 behavior on unchanged content — a snapshot's existence is never taken as proof it was applied, but the historical corpus also never replays through notification paths merely because the new code exists. A legacy record graduates to a verified marker when its content actually changes (the normal apply path), or through the separately authorized O3-B2 reconciliation — implemented as a governed, plan-bound census (dry-run default; seeding requires a reviewed immutable plan + digest; every historical mismatch is refused and enumerated, never auto-repaired; marker seeding writes only the four marker fields and zero notification events). This
  reconciliation, and the follow-on governed historical re-derivation and
  repair it fed into, are now complete in production — see "O3 production
  rollout and closeout" in [docs/recall-operations.md](recall-operations.md),
  which is also the operational contract and runbook.
- **FDA detail completeness.** A listing-backed FDA record's contract includes its detail page. A changed existing record whose page fetch fails is _deferred whole_ (nothing archived or applied; the previous complete version stays authoritative; the job withholds the feed gate so the next run retries even on identical bytes). A new listing-backed record still founds its case from listing evidence (coverage invariant) but as `applied_degraded`, and every later run retries the page until it lands — the richer payload is then archived honestly (same content hash, second snapshot) and its transition applied. RSS-only records quarantine without the page, as always.
- **Retries / errors:** per-adapter isolation — an FSIS 403 storm must not stall FDA polling. Bounded retries with backoff inside a run; a failed run is just recorded (IngestRun) and the next scheduled run tries again — at 30–60 min cadence, sophisticated retry orchestration is unnecessary. A record that fails parsing is quarantined _with its snapshot retained_ and surfaced in run output — never silently dropped (a parse bug must not hide a recall; the quarantine list is the bug queue). Within a run, per-item errors are isolated (one poison item cannot abort the feed) and counted; the job layer downgrades the run to `partial` for any unfinished item and fails it loudly when more than 20% of records fail (the existing quarantine-spike threshold, widened to cover isolated item errors).
...
- **Illness-report semantics** are owned by [recall-illness-status.md](recall-illness-status.md), which is the authoritative home for the derivation, the four-state contract (`src/domain/illness-status.ts`), the shopper copy, and the 2026-09-18 corpus audit behind them. In outline: a trustworthy count, a report without one, an explicit denial of **illnesses**, or unknown — and unknown renders nothing, because absence is never a zero. A denial of injuries or adverse reactions is not a denial of illnesses, and neither is a positive injury or adverse-reaction report an illness report; both stay in the source's own words. Disease education ("…can cause salmonellosis…"), healthcare advice, and discovery prose ("problem was discovered during surveillance…") are excluded from illness reporting; education may appear separately as "Health risk". `reportsIllness` on the projection is derived from the same contract, so material-change detection and Recall Detail cannot disagree.
...
- **Packaging is not foreign-material evidence.** `Product Contamination`
  ...
  container word cannot outrun it. **P3A removed the dead duplicate:** the
  unused `foreignMaterialAgent` bare-keyword helper and its private
  `FOREIGN_MATERIAL_SPECIFIC` vocabulary were deleted from
  `server/fda/parse.ts` (verified: no caller, import, test, or public
  contract) — FDA normalized output byte-identical across all 163 recorded
  announcements after the deletion. Within `Product Contamination`, supported

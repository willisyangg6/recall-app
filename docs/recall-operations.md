...
| Freshness heartbeat | `npm run ops:heartbeat`    | LAST step of `scheduled-ingest`       | Dead-man alerting (P2B7S). Reads the database and pings an external monitor **only** when both agency channels are within the 90-minute SLO, so reaching the last step is not itself the signal. Never writes, never fails the run, never prints a URL. **Not activated** — prints `NOT CONFIGURED` until `HEARTBEAT_URL` is set. Alerting is owned by docs/recall-production-runbook.md §16. |
...
Both declare every production secret **per step** rather than at job scope,
so `npm ci` never sees the service-role key, and both SHA-pin their actions;

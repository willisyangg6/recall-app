...
**Affected-product structure [VERIFIED on the recorded corpus and on bounded archived excerpts, 2026-09-04]:** the announcement body states its affected packages in four structurally distinct shapes, and which one a notice uses decides who owns its codes.

- **A named lead-in over an identifier list.** A paragraph names one product and ends by handing off ("The recalled Kroger Raw Colossal EZ Peel Shrimp net wt. 2lbs., is packaged in … and **has the following codes:**"), and the `<ul>` beneath it holds that product's own `UPC …, lot code …, Best If Used By: …` tuples. The AquaStar combined announcement carries three such structures. The lead-in `<p>` is the source's statement of ownership; the association is structural, and it is read only when the notice declares two or more such lists (with one, the list and the recall cover the same population).
- **A table with no Product column.** Two shapes exist and they must not be confused. **Row-oriented** (King Arthur: `Best Used By | Lot Code`; MedTech: `Item UPC | Lot # | Exp. Date` with a `rowspan` barcode) — each `<tr>` is one real affected row, whose facts belong together even though the notice names no product per row. **Column-oriented** (Wawona: each `<th>` is `BEST IF USED BY 09/23/2023` and the cells beneath it are that date's lot codes) — a `<tr>` is a run of grid cells whose codes belong to four different dates, and reading it as a row groups codes the source never grouped. A single-column table whose `<th>` is really a caption ("… Unbleached All-Purpose Flour 25 lb. UPC: 071012012503 Costco only") is a third variant: its header carries product identity this parser does not read.
- **Declared product rows with codes stated at recall level.** The rows name products; a separate sentence names codes without attributing them to any row.
- **Genuinely recall-level codes.** The notice states codes and names no product row for them at all (Twin Sisters Creamery's eight cheese batch codes; FSIS 103-2019's seven lot codes).

See [recall-feed-usability.md](recall-feed-usability.md), "A. Row-owned lot and batch codes — P3C-2", for the owner of each shape.

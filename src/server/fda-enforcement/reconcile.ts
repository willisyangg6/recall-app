/**
 * FDA announcement ↔ enforcement reconciliation — the canonical run, shared
 * verbatim by the manual CLI (scripts/reconcile-fda-enforcement.ts) and the
 * scheduled enforcement job. Matching (match.ts) and enrichment (enrich.ts)
 * are untouched: this module only owns corpus loading, the case sweep, and
 * reporting.
 *
 * Every run works from the COMPLETE official corpus — the openFDA bulk
 * export, one ~5.5 MB download naming the whole dataset, which is both
 ...
 * already-matched record. Reconciliation is idempotent; re-runs converge.
 */
...
export interface EnforcementCorpus {
  ...
  /** The bulk export's own date stamp; null for local files. */
  exportDate: string | null;
  totalRecords: number;
}

/** Offline corpus: a saved bulk-export JSON (optionally gzipped). */
export function loadEnforcementCorpusFromFile(path: string): EnforcementCorpus {
  const bytes = readFileSync(path);
  const text = path.endsWith('.gz') ? gunzipSync(bytes).toString('utf8') : bytes.toString('utf8');
  const parsed = JSON.parse(text);
  const records: OpenFdaEnforcementRaw[] = Array.isArray(parsed) ? parsed : parsed.results;
  return { records, exportDate: null, totalRecords: records.length };
}

/** The official bulk export: manifest lookup, then one zip download. */
export async function downloadEnforcementCorpus(
  fetchImpl: typeof fetch = fetch,
): Promise<EnforcementCorpus> {
  const manifest = await fetchEnforcementBulkUrl(fetchImpl);
  console.log(
  ...
  // The export is a single-file zip; minimal zip reading — locate the first
  // local file header and inflate it.
  ...
  const records = JSON.parse(json.toString('utf8')).results as OpenFdaEnforcementRaw[];
  return { records, exportDate: manifest.exportDate, totalRecords: manifest.totalRecords };
}
...
export interface ReconcileStats {
  ...
  conflicts: number;
  singleClassCases: number;
  mixedClassCases: number;
  mixedExposedAsScalar: number;
  assignments: number;
  reclassifications: number;
  notifications: { delivered: number; suppressed: number };
...
export async function reconcileFdaEnforcement(
  ...
  let quarantined = 0;
  for (const raw of options.corpus.records) {
    try {
  ...
  return {
    enforcementRecords: records.length,
    quarantined,
    casesExamined: summary.casesExamined,
    ...
    singleClassCases: summary.singleClassCases,
    mixedClassCases: summary.mixedClassCases,
    mixedExposedAsScalar: summary.mixedExposedAsScalar,
    assignments: summary.assignments,
    reclassifications: summary.reclassifications,

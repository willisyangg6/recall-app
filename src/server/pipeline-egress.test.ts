 ...
 * changed run must consult the narrow gate slice (identity + applied marker,
 * still never `normalized`), and reach for a full source-record row ONLY
 * where the expansion evidence guard genuinely reads the normalized payload.
 * Behavior itself (cases, snapshots, notifications) is pinned by
 * pipeline.test.ts and must not change — this file asserts the reads, not
 * new semantics.
 */
...
function countingStore(): { store: RecallStore; counts: Record<string, number> } {
  ...
  const counts: Record<string, number> = { full: 0, gate: 0 };
  ...
  wrapped.getSourceRecordGateByNativeId = (system, nativeId) => {
    counts.gate += 1;
    return store.getSourceRecordGateByNativeId(system, nativeId);
  };
...
test('re-ingesting an unchanged feed reads identity slices only, never full source rows', async () => {
  ...
  // run — is exactly one narrow gate lookup per record and zero full-row
  // reads (the snapshot-meta read stays a single narrow row as before).
